...
def test_1_tool_matches_real_open_path():
    ...
    res = tool.compute(direction="LONG", stop_atr_mult=1.5, tp_trigger_r=1.0,
...
def test_2_direction_mirror_and_validation():
    ...
    lng = tool.compute(direction="LONG", stop_atr_mult=1.5, tp_trigger_r=1.0)
    sht = tool.compute(direction="SHORT", stop_atr_mult=1.5, tp_trigger_r=1.0)

    assert lng["stop_price"] < ENTRY < sht["stop_price"], (lng, sht)
    assert lng["tp_price"] > ENTRY > sht["tp_price"], (lng, sht)
    assert abs(lng["r_unit_usd"] - sht["r_unit_usd"]) < 0.01

    bad = tool.compute(direction="SIDEWAYS", stop_atr_mult=1.5, tp_trigger_r=1.0)
...
def test_3_out_of_range_multipliers_clamped():
    ...
    res = tool.compute(direction="LONG", stop_atr_mult=9.0, tp_trigger_r=99.0)
    assert res["stop_atr_mult"] == risk.ai_stop_atr_mult_max, res
    assert res["tp_trigger_r"] == risk.ai_tp_trigger_r_max, res
    assert len(res["warnings"]) >= 2, res

    low = tool.compute(direction="LONG", stop_atr_mult=0.1, tp_trigger_r=0.01)
    assert low["stop_atr_mult"] == risk.ai_stop_atr_mult_min, low
    assert low["tp_trigger_r"] == risk.ai_tp_trigger_r_min, low

    junk = tool.compute(direction="LONG", stop_atr_mult="很宽", tp_trigger_r=None)
    assert junk["stop_atr_mult"] == risk.ai_stop_atr_mult_min, junk
    assert junk["tp_trigger_r"] == risk.tp_trigger_r, junk
...
def test_4_loss_scales_with_size_and_leverage():
    ...
    small = tool.compute(direction="LONG", stop_atr_mult=1.0, tp_trigger_r=1.0,
                         position_size_hint="25%", leverage=2)
    big = tool.compute(direction="LONG", stop_atr_mult=1.0, tp_trigger_r=1.0,
    ...
    # 止损放宽一倍 → R 与亏损同步翻倍
    wide = tool.compute(direction="LONG", stop_atr_mult=2.0, tp_trigger_r=1.0,
                        position_size_hint="25%", leverage=2)
...
def test_5_liquidation_constrains_r():
    ...
    res = tool.compute(direction="LONG", stop_atr_mult=3.0, tp_trigger_r=1.0,
    ...
    # TP 必须基于收窄后的真实 R
    assert abs(res["tp_price"] - (ENTRY + res["r_unit_usd"])) < 0.02, res
...
def test_6_schema_shape():
    ...
    for key in ("direction", "stop_atr_mult", "tp_trigger_r",
                "position_size_hint", "leverage"):
        assert key in params["properties"], key
    assert set(params["required"]) == {"direction", "stop_atr_mult", "tp_trigger_r"}
        ...
        "tool_calls": [{
            ...
            "function": {
                ...
                "arguments": json.dumps(args or {
                    "direction": "LONG", "stop_atr_mult": 1.5, "tp_trigger_r": 1.0,
                }),
            },
        }],
    }
