...
Trading AI 开仓时需要自己定止损宽度（即 1R），但它看不到带标签的绝对价格，也不该
看到——一旦让模型直接输出价位，它可能幻觉出一个贴着强平价的数字。折中方案是让它
只输出「相对量」（止损 = ATR × n），再通过本工具把相对量换算成真实价位和真实亏损
金额反馈给它，由它确认或调整。

持仓期间不再有第二个工具：保本 / 移动止损 / 落袋全由 AI 在每个 tick 用
action（平仓 / 减仓）和 stop_r（把硬止损推到某个 R 位置）直接表达。
...
def build_tool_schema(
    ...
    return [{
        ...
        "function": {
            "name": TOOL_NAME,
            "description": (
                "把止损的 ATR 倍数换算成真实止损价、1R 金额和打到止损的真实亏损。"
                "在决定 stop_atr_mult 之前必须调用，"
            ...
            "parameters": {
                ...
                "required": ["direction", "stop_atr_mult"],
            },
        },
    }]
...
class RiskLevelTool:
    ...
    def schema(self) -> List[Dict[str, Any]]:
        r = self.risk_cfg
        return build_tool_schema(r.ai_stop_atr_mult_min, r.ai_stop_atr_mult_max)
    ...
    def compute(
        ...
        **_ignored,
        ...
        # 给模型一个 R 刻度的参照：+1R 对应的价格与金额，便于之后用 stop_r 推理
        sign = 1 if is_long else -1
        one_r_price = self.entry_price + sign * r_unit

        warnings = [w for w in (stop_warn,) if w]
        ...
        return {
            ...
            "price_at_plus_1r": round(one_r_price, 2),
            "profit_at_plus_1r_usd": round(loss_at_stop, 2),
            "warnings": warnings,
        }
        ...
        for c in self.call_log:
            ...
            if "error" in res:
                ...
                parts.append(
                    ...
                    f"(${res['stop_price']:,.0f}) 1R=${res['r_unit_usd']:,.0f} "
                    f"亏损=${res['loss_at_stop_usd']:,.0f}"
